...
| Phase | What happens |
|---|---|
| `preparing` | a git worktree and branch per card. A resumed card reuses its worktree and commits; one with dependencies is cut from a fresh fetch of the base. |
...
| `fixing` | on the `fix` findings route only: findings return to the worker, at most **twice**, then the gates run again. |
