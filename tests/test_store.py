...
def test_an_existing_database_migrates_operator_author_rows_to_operator(tmp_path):
    """a board.db stuck at migration 15 still has the internal author key "operator" on its
    ...
    conn.execute(
        ...
        "VALUES ('cm1','c1','operator','an old note','t')"
    )
    ...
    conn.execute(
        ...
        "VALUES ('m1','b1','operator','build the thing','t')"
    )
    conn.commit()
    conn.close()
