// asks the REAL computeColumnFit (ui_base's pile.js) which regime each demo column lands
...
import {installStubDom, uiBaseAsset} from '../js/dom_stub.mjs';

const root = new URL('../../', import.meta.url);
installStubDom({fetchImpl: () => new Promise(() => {})});
// computeColumnFit lives in ui_base's pile.js; columns.js is still loaded because pile.js reads
// the gap custom properties through it and the two are only meaningful together
const src = [uiBaseAsset(root, 'pile.js'),
  readFileSync(new URL('smortboard/ui/columns.js', root), 'utf8')].join('\n;\n');
const {computeColumnFit} = new Function(`${src}\n;return {computeColumnFit};`)();
