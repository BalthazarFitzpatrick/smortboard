...
def _stopped(store: Store, state: LifecycleResult) -> LifecycleResult:
    """operator pulled the run. the card keeps its worktree and commits for a later run to resume -
    ...
    a claim about the work - it is operator's own decision, not something that went wrong.
    """
    store.update_card(state.card_id, review_flag=True)
    _note(store, state.card_id, "Stopped by operator.")
